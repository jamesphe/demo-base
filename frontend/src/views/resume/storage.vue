...
export default {
  ...
  methods: {
    ...
    async handleMove(row) {
      try {
        const newLocation = row.location === 'local' ? 'cloud' : 'local'
        const targetText = newLocation === 'cloud' ? '云端' : '本地'
        await this.$confirm(`确认将文件移至${targetText}存储?`, '提示', {
        ...
        if (index > -1) {
          this.$set(this.fileList[index], 'location', newLocation)
        }
